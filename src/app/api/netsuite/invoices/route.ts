...
const FiltersSchema = z.object({
  ...
  sortBy: z.string().default("invoice_created_date"),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const parsed = FiltersSchema.parse({
      ...
      sortBy: searchParams.get("sortBy") || "invoice_created_date",
      sortDirection: searchParams.get("sortDirection") || "desc",
      searchText: searchParams.get("searchText"),
    });

    let query = supabase
      .from("mv_invoices_with_usd")
      .select("*");
    ...
    if (parsed.searchText && parsed.searchText.trim().length > 0) {
      ...
      query = query.or(
        `invoice_customer_name.ilike.%${s}%,invoice_transaction_id.ilike.%${s}%,invoice_status.ilike.%${s}%`
      );
    }

    // Apply default sorting by created date
    query = query.order("invoice_created_date", {
      ascending: false,
      nullsFirst: false
    });
    ...
    if (countError) {
      console.error("❌ [NetSuite Invoices] Count error:", countError);
      return NextResponse.json(
        { error: "Failed to count invoices", details: countError.message },
        { status: 500 }
      );
    }
    ...
    if (error) {
      console.error("❌ [NetSuite Invoices] Database error:", error);
      return NextResponse.json(
    ...
    console.error("❌ [NetSuite Invoices] Error:", error);
    return NextResponse.json(
