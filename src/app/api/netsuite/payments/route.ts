...
const FiltersSchema = z.object({
  ...
  sortBy: z.string().default("payment_created_date"),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const parsed = FiltersSchema.parse({
      ...
      sortBy: searchParams.get("sortBy") || "payment_created_date",
      sortDirection: searchParams.get("sortDirection") || "desc",
      searchText: searchParams.get("searchText"),
    });

    let query = supabase
      .from("mv_payments_with_usd")
      .select("*");
    ...
    if (parsed.searchText && parsed.searchText.trim().length > 0) {
      ...
      query = query.or(
        `payment_customer_name.ilike.%${s}%,payment_transaction_id.ilike.%${s}%,payment_status.ilike.%${s}%`
      );
    }

    // Apply default sorting by created date
    query = query.order("payment_created_date", {
      ascending: false,
      nullsFirst: false
    });
    ...
    if (countError) {
      console.error("❌ [NetSuite Payments] Count error:", countError);
      return NextResponse.json(
        { error: "Failed to count payments", details: countError.message },
        { status: 500 }
      );
    }
    ...
    if (error) {
      console.error("❌ [NetSuite Payments] Database error:", error);
      return NextResponse.json(
    ...
    console.error("❌ [NetSuite Payments] Error:", error);
    return NextResponse.json(
